<!doctype html>
<html xmlns="http://www.w3.org/1999/xhtml" xmlns:v="urn:schemas-microsoft-com:vml"
xmlns:o="urn:schemas-microsoft-com:office:office">

<head>
	<title></title>
	<meta http-equiv="X-UA-Compatible" content="IE=edge">
	<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
	<meta name="viewport" content="width=device-width, initial-scale=1.0">

</head>



<body>
	<div style="background-color:#fff;margin:0 auto 0 auto;padding:30px 0 30px 0;color:#4f565d;font-size:13px;line-height:20px;font-family:'Helvetica Neue',Arial,sans-serif;text-align:left;">
		<center>
			<table style="width:550px;text-align:center">
				<tbody>
					<tr>
						<td style="padding:0 0 20px 0;border-bottom:1px solid #e9edee;">
							<a href="https://www.xero.com/us/" style="display:block; margin:0 auto;" target="_blank">
								<img src="https://image.ibb.co/egY2EG/logo.png" width="300" height="75" alt="VLU" style="border: 0px;">
							</a>
						</td>
					</tr>
					<tr>
						<td colspan="2" style="padding:30px 0; border-bottom:1px solid #e9edee;">
							<p style="color:#1d2227;line-height:28px;font-size:24px;margin:12px 10px 20px 10px;font-weight:400;">Xin chào {{$representative->name}}</p>
							<p style="font-size: 20px; margin:0 10px 10px 10px;padding:0;">Công ty <strong>{{$company->name}}</strong></p>
							<p>
							</td>
						</tr>
						<tr>
							<td colspan="2" style="padding:30px 0;">
								<span style="color:#1d2227;line-height:28px;font-size:21px;font-weight:400;">Sau khi xem xét, chúng tôi <span style="color: #ff1a1a;">chưa được chấp thuận</span> được tin tuyển dụng.</span> 
								<br>
								<p style="color:#1d2227;line-height:28px;font-size:21px;margin:12px 10px 20px 10px;font-weight:400;">Vì lý do:  {{$message1}}</p>
								<br>
								<span style="color:#1d2227;line-height:28px;font-size:21px;font-weight:400;">Tin tuyển dụng:<strong> {{$recruitment->title}}</strong> </span> 
								<br>
								<span style="color:#1d2227;line-height:28px;font-size:21px;font-weight:400;">Đã bởi: {{$representative->name}} lúc {{$recruitment->created_at}} </span> 
								<p style="color:#1d2227;line-height:28px;font-size:21px;margin:12px 10px 20px 10px;font-weight:400;">Bạn có thể chỉnh sửa lại tin tuyển dụng và sau đó chúng tôi sẽ xem sét và chấp thuận</p>
								<p style="color:#1d2227;line-height:28px;font-size:21px;margin:12px 10px 20px 10px;font-weight:400;">Cảm ơn</p>

								{{-- <p style="font-size: 18px;padding:0;"><strong>Username: </strong> {{$account->username}} </p>
								<p> --}}

									{{-- <p style="margin:0 10px 10px 10px;padding:0;">Rất vui vì bạn đã tham gia VanLang E-Recruiting, ấn nút phía dưới để cập nhật mật khẩu.</p>

									<a style="display:inline-block;text-decoration:none;padding:15px 20px;border:1px solid #2baaed; background:#2baaed; border-radius:3px;color:white;font-weight:bold;" href="{{Route('representative.resetpassword', ['token' => $account -> remember_token]) }}" target="_blank">Cập nhật mật khẩu</a>

									

									<p>

									</p> --}}
								</td>
							</tr>
							<tr>
								<td colspan="2" style="padding:30px 0 0 0;border-top:1px solid #e9edee;color:#9b9fa5">
									Nếu bạn có bất kì thắc mắc nào, đừng ngần ngại liên hệ với chúng tôi tại <a style="color:#666d74;text-decoration:none;" href="#" target="_blank">jobee0210@gmail.com</a>

									<br>
									<br>
									<br>
									<br>
									<br>
									<br>

									Đại Học Văn Lang
									<br>
									Số 45, Nguyễn Khắc Nhu, P. Cô Giang, Quận 1, TP. Hồ Chí Minh.
									<br>
									<p>Email được gửi bởi VanLang E-Recruiting<br>Copyrights © 2017 All Rights Reserved by VanLang E-Recruiting.</p>
								</td>
							</tr>
						</tbody>
					</table>
				</center>
			</div>
		</body>

		</html>